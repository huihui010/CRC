...
namespace ManageCustomer.Controllers
{
    ...
    [ApiController]
    public class ClientLinkmanController : ControllerBase
    {
        //客户联系人管理  查 增 改 删

        //客户联系人管理  查
        [HttpGet]
        public ActionResult GetLinkmanData()
        {
            //连接数据库，打开数据库连接
            ...
            else
            {
                ...
                {
                    //sql语句参数化，防止sql注入问题
                    MySqlCommand cmd1 = new MySqlCommand("select * from client_contact", conn);
                    //执行sql命令 结果填充DataSet
                ...
                else
                {
                    ...
                    if (re != "outdate" && re != "error" && re != "null")
                    {
                        ...
                        if (res.Rows.Count > 0)
                        {
                            ...
                            //sql语句参数化，防止sql注入问题
                            MySqlCommand cmd1 = new MySqlCommand("select * from client_contact where clientID in (select clientID from clientinfo where salesmanID=@salesmanID)", conn);
        ...
        //客户联系人管理  增
        [HttpPost]
        public ActionResult InsertLinkmanData([FromBody] JObject value)
        {
            string clientID = value["clientID"].ToString();
            string contactName = value["contactName"].ToString();
            string contactPhone = value["contactPhone"].ToString();
            string contactProfession;
            string contactAddress;
            if (value["contactProfession"] == null)
            {
                contactProfession = null;
            }
            else
            {
                contactProfession = value["contactProfession"].ToString();
            }
            if (value["contactAddress"] == null)
            {
                contactAddress = null;
            }
            else
            {
                contactAddress = value["contactAddress"].ToString();
            }
            ...
            conn.Open();
            //sql语句参数化，防止sql注入问题
            MySqlCommand cmd = new MySqlCommand($"insert into client_contact(clientID,contactName,contactPhone,contactProfession,contactAddress,isDeleted) values (@clientID,@contactName,@contactPhone,@contactProfession,@contactAddress,'0')", conn);
            MySqlParameter[] msp = new MySqlParameter[]
            {
                new MySqlParameter("@clientID",clientID),
                new MySqlParameter("@contactName",contactName),
                new MySqlParameter("@contactPhone",contactPhone),
                new MySqlParameter("@contactProfession",contactProfession),
                new MySqlParameter("@contactAddress",contactAddress)
        ...
        public ActionResult GetUpdateData([FromBody] JObject value)
        {
            ...
            conn.Open();
            //sql语句参数化，防止sql注入问题
            MySqlCommand cmd = new MySqlCommand($"select * from client_contact where contactID=@contactID", conn);
            MySqlParameter[] msp = new MySqlParameter[]
            {
        ...
        //客户联系人管理  改
        [HttpPut]
        public ActionResult UpdateLinkmanData([FromBody] JObject value)
        {
            string contactID = value["contactID"].ToString();
            string clientID = value["clientID"].ToString();
            string contactName = value["contactName"].ToString();
            ...
            conn.Open();
            //sql语句参数化，防止sql注入问题
            MySqlCommand cmd = new MySqlCommand($"update client_contact set clientID=@clientID,contactName=@contactName,contactPhone=@contactPhone,contactProfession=@contactProfession,contactAddress=@contactAddress where contactID=@contactID", conn);
            MySqlParameter[] msp = new MySqlParameter[]
            {
                new MySqlParameter("@clientID",clientID),
                new MySqlParameter("@contactName",contactName),
        ...
        //客户联系人管理  删
        [HttpDelete]
        public ActionResult RemoveLinkmanData([FromBody] JObject value)
        {
            string contactID = value["contactID"].ToString();
            string connstr = "server=127.0.0.1;port=3306;user=root;password=123456;database=crm;Charset=utf8;";
            using MySqlConnection conn = new MySqlConnection(connstr);
            ...
            MySqlCommand cmd = new MySqlCommand($"update client_contact set isDeleted='1' where contactID=@contactID", conn);
            MySqlParameter[] msp = new MySqlParameter[]
            {
                new MySqlParameter("@contactID",contactID),
            };
